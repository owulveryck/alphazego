...
//     [MCTSNode.UCB1] score until a leaf node is reached.
//  2. Expansion: add one new child for an untried move ([MCTSNode.Expand]).
//  3. Simulation: random rollout until a terminal state ([MCTSNode.Simulate]).
...
// [board.Evaluator] (reseau de neurones) et une constante d'exploration cpuct:
//
...
// Each iteration performs:
//
//  1. Selection: descend the tree using [MCTSNode.PUCT] (avec priors du
//     policy network) au lieu de UCB1.
//  2. Expansion + Evaluation: appel unique a [board.Evaluator.Evaluate] pour
//     obtenir policy et value. Tous les enfants sont crees d'un coup avec
//     leurs priors ([MCTSNode.ExpandAll]).
